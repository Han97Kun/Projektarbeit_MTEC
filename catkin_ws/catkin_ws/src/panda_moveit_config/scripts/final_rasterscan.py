...
def setup_environment(scene, commander):
    ...
    # sizes of the gamma source and the sponge (just height of the sponge)
    height_sponge          = 0.03 # height of the sponge in meters (2.9cm)  #1.4cm ist die mitgelieferte box
...
def raster_menu(ctx):
    ...
    while not rospy.is_shutdown():
        ...
        while repeat and not rospy.is_shutdown():
            ...
            if raster_choice == '1':
                config = {
                    "raster_size": 51,             # Anzahl Punkte
                    "delta": 0.002,                 # in m
                    "heights": [0.08, 0.09, 0.1, 0.12],     # 5 mm und 10 mm über Quelle
                    "measurement_time_seconds": 9, # Messdauer pro Punkt
            ...
            if raster_choice == '2':
                config = {
                    ...
                    "delta": 0.001,                 # in m
                    "heights": [0.001, 0.002, 0.003, 0.004],     # 5 mm und 10 mm über Quelle
                    "measurement_time_seconds": 9, # Messdauer pro Punkt
                    "ocr_interval_s": 0.5,        # 4 Hz OCR
                    "enable_ocr": True,
                }
